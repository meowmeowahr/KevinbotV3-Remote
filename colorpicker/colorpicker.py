...
class ColorPicker(QWidget):
    ...
    def getRGB(self, range=255):
        ...
        return round(r * (range / 255.0)), round(g * (range / 255.0)), round(b * (range / 255.0))
    ...
    def hsvChanged(self):
        ...
        self.ui.color_view.setStyleSheet(
            "border-radius: 5px;"
            f"background-color: qlineargradient(x1:1, x2:0, stop:0 hsl({h}%,100%,50%), stop:1 #fff);")
    ...
    def _setHSV(self, c):
        self.ui.hue_selector.move(int(7), int((100 - c[0]) * 1.85))
        self.ui.color_view.setStyleSheet(
            f"border-radius: 5px;background-color: qlineargradient(x1:1, x2:0, stop:0 hsl({c[0]}%,100%,50%), stop:1 "
            "#fff);")
    ...
    def hex2rgb(self, hex):
        ...
        # if hex has any non-hex characters, return (0,0,0)
        if not all(c in "0123456789abcdef" for c in hex): return (0, 0, 0)
